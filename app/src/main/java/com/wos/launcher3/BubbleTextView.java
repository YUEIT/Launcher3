...
public class BubbleTextView extends TextView {
    ...
    private static final int TRANSPARENT_LEN = 5;
    private void initTransparent(){
        Paint mPaint = getPaint();
        CharSequence text = this.getText();
        if (text.length()> TRANSPARENT_LEN){
            float textWidth = mPaint.measureText(text, 0, TRANSPARENT_LEN);
            this.setText(text.subSequence(0, TRANSPARENT_LEN));
            LinearGradient mLinearGradient = new LinearGradient(0, 0, 100, 0, new int[] {
                    Color.WHITE, Color.BLUE }, null, Shader.TileMode.REPEAT);
            mPaint.setShader(mLinearGradient);
        }else{
            float textWidth = mPaint.measureText(text, 0, text.length());
            LinearGradient mLinearGradient = new LinearGradient(0, 0, textWidth, 0, new int[] {
                    Color.WHITE, Color.BLUE}, null, Shader.TileMode.REPEAT);
            mPaint.setShader(mLinearGradient);
        }
    }
    ...
    public void draw(Canvas canvas) {
        if (!mCustomShadowsEnabled) {
            super.draw(canvas);
            //A:luobiao@wind-mobi.com 2015-8-14 Added for wos unread message feature.@{
