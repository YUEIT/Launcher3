 ...
 */
public class WOSUnreadLoader extends BroadcastReceiver {
    ...
    public final static String ACTION_UNREAD_CHANGED = "com.mediatek.action.UNREAD_CHANGED";
    public final static String EXTRA_UNREAD_COMPONENT = "com.mediatek.intent.extra.UNREAD_COMPONENT";
    public final static String EXTRA_UNREAD_NUMBER = "com.mediatek.intent.extra.UNREAD_NUMBER";
    public final static String ACTION_MMS_CONTENT_OBSERVER = "com.wos.launcher3.action.MMS_CONTENT_OBSERVER";
    public final static String ACTION_CALL_CONTENT_OBSERVER = "com.wos.launcher3.action.CALL_CONTENT_OBSERVER";
    private Context mContext;

    private WeakReference<UnreadCallbacks> mCallbacks;

    public WOSUnreadLoader(Context context) {
        mContext = context;
    }
    ...
    public void onReceive(final Context context, final Intent intent) {
        ...
        */
        if(ACTION_MMS_CONTENT_OBSERVER.equals(action)){
            final ComponentName componentName = new ComponentName("com.android.mms", "com.android.mms.ui.BootActivity");
            int unreadNum = intent.getIntExtra("unreadNum", -1);
            unreadChanged(componentName, unreadNum);
        }
        if(ACTION_CALL_CONTENT_OBSERVER.equals(action)){
            final ComponentName componentName = new ComponentName("com.android.dialer", "com.android.dialer.DialtactsActivity");
            int unreadNum = intent.getIntExtra("unreadNum", -1);
            unreadChanged(componentName, unreadNum);
        }
    ...
    public void unreadChanged (ComponentName componentName , int unreadNum){
        Log.d("LUOBIAO", "WOSUnreadLoader:unreadChanged:componentName="+componentName+",unreadNum"+unreadNum);
        if (mCallbacks != null && componentName != null && unreadNum != -1) {
            final int index = supportUnreadFeature(componentName);
            if (index >= 0) {
                boolean ret = setUnreadNumberAt(index, unreadNum);
                if (ret) {
                    final UnreadCallbacks callbacks = mCallbacks.get();
                    if (callbacks != null) {
                        Log.d("LUOBIAO", "WOSUnreadLoader:unreadChanged:callbacks.bindComponentUnreadChanged");
                        callbacks.bindComponentUnreadChanged(componentName, unreadNum);
                    }
                }
            }
